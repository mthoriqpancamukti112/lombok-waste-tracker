...
const BottomBar: React.FC<BottomBarProps> = ({ activeTab, onTabClick, onAuthClick, onCreateClick, user }) => {
    ...
    const ReportsItem = () => (
        ...
        >
            {activeTab === 'reports' ? <ListCheckSolid className="w-7 h-7" /> : <ListCheck className="w-7 h-7" />}
            <span className="text-[11px] font-bold tracking-tight">Report List</span>
        </button>
    );

    /** Right nav item — Profile/Login */
    const AuthItem = () => {
        if (isLoggedIn) {
            return (
                <button
                ...
                >
                    <div className={`w-7 h-7 rounded-full overflow-hidden border-2 transition-all ${activeTab === 'profile' ? 'border-[#a7e94a] scale-110' : 'border-transparent'}`}>
                        <img src={avatarSrc} alt={user?.name} className="w-full h-full object-cover" />
                    </div>
                    <span className={`text-[11px] font-bold tracking-tight max-w-[72px] truncate ${activeTab === 'profile' ? 'text-[#a7e94a]' : 'text-slate-400'}`}>
        ...
        return (
            ...
            >
                ...
                <span className="text-[11px] font-bold tracking-tight">Login</span>
            </button>
        );
    };
    ...
    const ReportsItemDesktop = () => (
        ...
        >
            {activeTab === 'reports' ? <ListCheckSolid className="w-6 h-6" /> : <ListCheck className="w-6 h-6" />}
            <span className="text-[11px] font-bold tracking-tight">Report List</span>
        </button>
    );
    ...
    const AuthItemDesktop = () => {
        if (isLoggedIn) {
            return (
                ...
                >
                    <div className={`w-6 h-6 rounded-full overflow-hidden border-2 transition-all ${activeTab === 'profile' ? 'border-[#a7e94a] scale-110' : 'border-transparent'}`}>
                        <img src={avatarSrc} alt={user?.name} className="w-full h-full object-cover" />
                    </div>
                    <span className={`text-[11px] font-bold tracking-tight max-w-[64px] truncate ${activeTab === 'profile' ? 'text-[#a7e94a]' : 'text-slate-400'}`}>
                        {user?.name?.split(' ')[0] || 'Profile'}
                    </span>
                </button>
            );
    ...
    return (
        <>
            ...
            >
                <div className={`transition-all duration-300 ease-out ${dockVisible ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                ...
                <div className={`flex items-center justify-center w-24 h-7 bg-[#a7e94a] backdrop-blur-sm rounded-t-2xl shadow-lg transition-all duration-300 cursor-pointer ${dockVisible ? 'opacity-50' : 'opacity-100 hover:opacity-80'}`}>
                    <ChevronUp className={`w-6 h-6 text-white transition-transform duration-300 ${dockVisible ? 'rotate-180' : 'rotate-0'}`} />
                </div>
            </div>
        </>
    );
