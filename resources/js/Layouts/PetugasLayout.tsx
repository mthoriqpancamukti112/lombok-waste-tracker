...
export default function PetugasLayout({
    ...
    // Data Menu Sidebar KHUSUS PETUGAS
    const sidebarMenus = [
        {
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex transition-colors duration-300">
            ...
            {/* Sidebar (Warna Khas: Amber / Orange Gelap) */}
            <aside
                className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-72 bg-amber-950 dark:bg-slate-950 text-white flex flex-col transition-transform duration-300 ease-in-out border-r border-transparent dark:border-slate-800 ${
                    isSidebarOpen
                        ? "translate-x-0"
            ...
            >
                {/* Logo */}
                <div className="h-20 flex flex-shrink-0 items-center px-6 border-b border-amber-900 dark:border-slate-800 justify-between lg:justify-start">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center text-amber-950 shadow-[0_0_15px_rgba(245,158,11,0.3)]">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-amber-300 dark:text-slate-400 font-bold uppercase tracking-widest">
                                {t.petugasPortal}
                            </p>
                        </div>
                    </div>
                ...
                <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1 custom-scrollbar">
                    <p className="px-4 text-[10px] font-bold text-amber-500 dark:text-slate-500 uppercase tracking-widest mb-3">
                        {t.petugasMainMenu}
                    </p>
                    {sidebarMenus.map((menu, index) => (
                        <Link
                            ...
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${
                                menu.active
                                    ? "bg-amber-500 dark:bg-amber-600 text-amber-950 dark:text-white shadow-md"
                                    : "text-amber-300 dark:text-slate-400 hover:bg-amber-800 dark:hover:bg-slate-800 hover:text-white"
                            }`}
                        >
                            <div
                            ...
                            >
                                ...
                                <div className="w-9 h-9 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 flex items-center justify-center font-black text-sm border-2 border-white dark:border-slate-800 shadow-sm overflow-hidden relative flex-shrink-0">
                                    ...
                                    <span
                                        ...
                                        className={`absolute bottom-0 right-0 w-2.5 h-2.5 border-2 border-white dark:border-slate-800 rounded-full z-20 ${
                                            auth.user?.petugas?.is_aktif
                                                ? "bg-emerald-500"
                                                : "bg-slate-400"
                                        }`}
                                ...
                                <>
                                    ...
                                    <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 z-50 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200 p-2">
                                        ...
                                        <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 mb-2">
                                            ...
                                            {/* Info Plat Nomor Kendaraan */}
                                            {auth.user?.petugas?.plat_nomor && (
                                        ...
                                        <Link
                                            ...
                                            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400 hover:bg-amber-50 dark:hover:bg-slate-700 transition-colors group"
                                            onClick={() =>
                                                setIsProfileMenuOpen(false)
                                            }
                                        >
                                            <User
                                                className="w-5 h-5 text-slate-400 dark:text-slate-500 group-hover:text-amber-500 dark:group-hover:text-amber-400"
                                                strokeWidth={1.5}
                                            />
