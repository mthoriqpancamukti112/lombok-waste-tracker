...
export default function Welcome({
    ...
    const [activePanel, setActivePanel] = useState<'none' | 'reports' | 'profile' | 'report-detail'>('none');
    ...
    const [initialReportLocation, setInitialReportLocation] = useState<{ lat: number; lng: number; address: string } | null>(null);

    const mapRef = useRef<{ centerOnUser: () => void; flyTo: (lat: number, lng: number) => void } | null>(null);
    ...
    useEffect(() => {
        if (auth.user) {
            const pendingLocation = sessionStorage.getItem('pendingReportLocation');
            if (pendingLocation) {
                try {
    ...
    useEffect(() => {
        const performSearch = async () => {
            ...
            try {
                ...
                const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(debouncedSearchQuery)}&format=json&addressdetails=1&limit=5&countrycodes=id`);
    ...
    const unreadNotifications = auth.user?.notifications?.filter((n: any) => !n.read_at) || [];
            ...
            <div className={`fixed inset-0 w-full overflow-hidden ${isDarkMode ? 'bg-slate-900' : 'bg-slate-50'} h-[100dvh]`}>
                ...
                <div className="absolute inset-0 z-0">
                    <MapComponent
                        ...
                        onReportSearchedLocation={() => {
                            if (!auth.user) {
                                if (searchedLocation) {
                                    sessionStorage.setItem('pendingReportLocation', JSON.stringify(searchedLocation));
                                }
                                openAuthModal("login");
                            } else {
                ...
                <div className="xl:hidden absolute top-0 inset-x-0 z-50 px-4 pt-5 flex flex-col gap-2.5">
                    <div className="relative">
                        <div className={`bg-white dark:bg-slate-800 shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-slate-100 dark:border-slate-700 rounded-2xl flex items-center px-4 py-1`}>
                            <input
                                type="text"
                                value={searchQuery}
                            ...
                            <div className="absolute top-14 inset-x-0 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50">
                                {searchResults.map((res) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-medium text-slate-700 dark:text-slate-300 line-clamp-2">{res.place_name}</span>
                                    </button>
                                ))}
                            </div>
                        )}
                        {showSearchResults && searchResults.length === 0 && !isSearching && searchQuery.length >= 3 && (
                            <div className="absolute top-14 inset-x-0 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50 p-4 text-center">
                                <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
                                    {lang === 'id' ? 'Lokasi tidak ditemukan' : 'Location not found'}
                                </span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        {auth.user && (
                            <div className="relative">
                                <button onClick={() => setShowNotifications(!showNotifications)} className={`${mobileBtnBase} bg-white text-slate-500 relative`}>
                                    {unreadNotifications.length > 0 ? <BellSolid className="w-[18px] h-[18px] text-[#a7e94a]" /> : <Bell className="w-[18px] h-[18px]" />}
                                ...
                                {showNotifications && (
                                    <div className="absolute top-12 right-0 w-[85vw] max-w-sm bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50 flex flex-col">
                                        <div className="p-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                                            ...
                                            {unreadNotifications.length > 0 && (
                                                <button onClick={handleMarkAllAsRead} className="text-[10px] font-bold text-[#a7e94a]">Tandai semua dibaca</button>
                                            )}
                                        </div>
                                                ...
                                                auth.user.notifications?.map((notif: any) => (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            if (notif.data.report_id) handleReportSelect(notif.data.report_id);
                                                            setShowNotifications(false);
                                                        }}
                                                    ...
                                                    >
                                                        <div className="flex gap-2">
                                                            <div className={`mt-1 w-1.5 h-1.5 shrink-0 rounded-full ${!notif.read_at ? 'bg-[#a7e94a]' : 'bg-transparent'}`} />
                                                            <div className="flex-1">
                                                                <p className={`text-[11px] ${!notif.read_at ? 'font-bold text-slate-800 dark:text-slate-200' : 'font-medium text-slate-600 dark:text-slate-400'}`}>
                                                                    {notif.data.message}
                                                                </p>
                                                                <p className="text-[9px] text-slate-400 mt-0.5">
                                                                    {new Date(notif.created_at).toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </button>
                            ...
                            <div className="absolute top-16 inset-x-0 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50">
                                {searchResults.map((res) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{res.place_name}</span>
                                    </button>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="flex-[3] flex items-center justify-end gap-2.5">
                        ...
                        {auth.user && (
                            <div className="relative">
                                ...
                                >
                                    {unreadNotifications.length > 0 ? <BellSolid className="w-5 h-5 text-[#a7e94a]" /> : <Bell className="w-5 h-5" />}
                                    {unreadNotifications.length > 0 && (
                                ...
                                {showNotifications && (
                                    <div className="absolute top-16 right-0 w-80 bg-white dark:bg-slate-800 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700 overflow-hidden z-50 flex flex-col">
                                        <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                                            ...
                                            {unreadNotifications.length > 0 && (
                                                <button onClick={handleMarkAllAsRead} className="text-[10px] font-bold text-[#a7e94a] hover:underline">Tandai semua dibaca</button>
                                            )}
                                        </div>
                                                ...
                                                auth.user.notifications?.map((notif: any) => (
                                                    ...
                                                    >
                                                        <div className="flex gap-3">
                                                            <div className={`mt-1.5 w-2 h-2 shrink-0 rounded-full ${!notif.read_at ? 'bg-[#a7e94a]' : 'bg-transparent'}`} />
                                                            <div className="flex-1">
                                                                <p className={`text-xs ${!notif.read_at ? 'font-bold text-slate-800 dark:text-slate-200' : 'font-medium text-slate-600 dark:text-slate-400'}`}>
                                                                    {notif.data.message}
                                                                </p>
